    ...
    public class Users {
        ...
        protected void onCreate() {
            ...
            this.created_at = LocalDateTime.now();  // 현재 시간을 생성 일시로 설정
            this.updated_at = LocalDateTime.now();  // 현재 시간을 업데이트 일시로 설정
            // 포인트 초기값 설정
            if (this.point == null) {
        ...
        protected void onUpdate() {
            this.updated_at = LocalDateTime.now();  // 업데이트가 발생할 때마다 현재 시간을 업데이트 일시로 설정
        }
    }